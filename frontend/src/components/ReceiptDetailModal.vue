  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
    <div class="w-full max-w-lg rounded-2xl bg-[#0f172a] p-4 text-white shadow-2xl sm:p-5">
      ...
      <div class="mt-4 grid gap-3 sm:grid-cols-2">
        <button class="rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-sm hover:bg-white/15" @click="$emit('pdf')">
          Salvar em PDF
        </button>
        <button class="rounded-xl bg-sky-600 px-4 py-3 text-sm text-white hover:bg-sky-500" @click="$emit('email')">
          Enviar por e-mail
        </button>
      </div>
    </div>
